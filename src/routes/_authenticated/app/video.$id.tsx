      ...
      {!v ? <div className="h-40 animate-pulse rounded-xl bg-card" /> : (
        <>
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-black">
            {signedUrl ? (
              ...
              <div className="grid aspect-video place-items-center text-sm text-muted-foreground">
                Externer Link: <a href={v.source_url} target="_blank" rel="noreferrer" className="ml-1 text-primary underline">öffnen</a>
              </div>
            ) : (
          ...
          <div className="rounded-2xl border border-border bg-card p-6">
            <div className="flex items-center gap-2 text-sm font-medium"><Wand2 className="h-4 w-4 text-primary" /> Neuen Schnitt starten</div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {(
                ...
                <button key={o.v} onClick={() => setMode(o.v)} className={`rounded-xl border p-4 text-left transition ${mode === o.v ? "border-primary bg-primary/10" : "border-border bg-background hover:border-primary/40"}`}>
            ...
            <button onClick={() => createJob.mutate()} disabled={createJob.isPending} className="mt-6 inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-60">
