          ...
          <div>
            ...
            {tpl && (
              <p className="mt-2 text-[10px] text-muted-foreground">
                Passt Modus, Format, Captions & Sound-Mood automatisch an — du kannst unten alles
                überschreiben.
              </p>
            )}
          </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(
                ...
                <button
                  key={o.v}
                  onClick={() => setMode(o.v)}
                  className={`rounded-lg border p-2 text-left ${mode === o.v ? "border-primary bg-primary/10" : "border-border bg-background hover:border-primary/40"}`}
                >
              ...
              <div className="flex gap-1">
                {(["9:16", "16:9", "1:1"] as const).map((a) => (
                  <button
                    key={a}
                    onClick={() => setAspect(a)}
                    className={`flex-1 rounded-md border px-2 py-1 text-xs ${aspect === a ? "border-primary bg-primary/10 text-primary" : "border-border hover:bg-secondary"}`}
                  >
                    {a}
                  </button>
                ))}
